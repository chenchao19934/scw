<template>
  <div class="wx-goodCard"
       @click="toPage(recommend)">
    <div class="wx-goodCard__imgWrap">
      <img :src="`http://woxfile.oss-cn-shenzhen.aliyuncs.com/uploadfile/article/${recommend.picPath}`"
           alt="">
    </div>
    <div class="wx-goodCard__textWrap">
      <p class="wx-goodCard__shopName ellipsis">{{ recommend.field1 }}</p>
      <p class="wx-goodCard__shopSlogan ellipsis">{{ recommend.field2 }}</p>
      <p class="wx-goodCard__shopAvgConsume">{{ recommend.field3 }}</p>
    </div>
  </div>
</template>
