<template>
  <div class="wx-home">
    <i-sidebar-menu :showSideMenu='isShowSideMenu'>
      <SideContent :logoImg="logoImg" slot="slidebarContent">
        <SideBarList>
          ...
          <SideBarListItem title="会员"
                           subTitle="Members"
                           icon="i-icon-person"
                           @click.native="$router.push({ name: 'memberCenter' })"></SideBarListItem>
          <!-- 顺德印象城bid=183 -->
  ...
  methods: {
    ...
    async signIn() {
      ...
      if (!lastSignTime || (new Date().getTime() - (lastSignTime * 1000) >= 1000 * 60 * 60 * 24)) {
        ...
        Velocity(document.querySelector('.wx-signIn'), {opacity: 0.5} ,{duration:1500});
        this.$Toast('您已经签到过了');
      }
    },
