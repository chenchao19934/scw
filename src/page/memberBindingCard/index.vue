...
<script>
import { TheHeader, IField, IRadio, IButton, IChecklist } from '@/components/index.js';
  ...
  methods: {
    ...
    submitDataBinding: _.debounce(
      async function submitDataBinding() {
        ...
        // 校验卡号
        if (!this.bindingCardForm.cardID && window.$bid !== '51') {
          this.$Toast('请填写您的卡号');
          return;
        }
        ...
        if (this.$route.query.bid !== '51') {
          ...
          if (!this.bindingCardForm.verificationCode) {
            ...
            return;
          } else if (!regexBindingVerificationCode.test(this.bindingCardForm.verificationCode)) {
