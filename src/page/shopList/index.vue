...
export default {
  ...
  methods: {
    ...
    async loadMore() {
      ...
      this.loading = true;
      const data = await service.shopIndex({
        curPos: this.count,
      ...
      if (data.errCode === 0) {
        ...
        if (data.result.shop) {
          for (let i = 0; i < data.result.shop.length; i++) {
            data.result.shop[
              i
            ].logoPath = `http://woxfile.oss-cn-shenzhen.aliyuncs.com/uploadfile/article/${
              data.result.shop[i].logoPath
            }`;
          }
        }

        for (let i = 0; i < data.result.yetai_list.length; i++) {
          data.result.yetai_list[
            i
          ].picPath = `http://woxfile.oss-cn-shenzhen.aliyuncs.com/uploadfile/article/${
            data.result.yetai_list[i].picPath
          }`;
        }
