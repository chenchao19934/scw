  ...
  <div class="wx-activity container">
    ...
    <div class="wx-listWrap"
      ...
      <ul v-show='!noData'
        ...
        <ActivityItem v-for='activity in activity_list'
                      :key="activity.id"
                      :activity="activity"
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.listHeight = document.documentElement.clientHeight - this.$refs.wrapper.getBoundingClientRect().top;
    });
  },
  ...
  methods: {
    ...
    async Activity() {
      ...
      if (data.errCode === -1) {
        ...
        this.activity_list = data.result.list;
      }
    },
